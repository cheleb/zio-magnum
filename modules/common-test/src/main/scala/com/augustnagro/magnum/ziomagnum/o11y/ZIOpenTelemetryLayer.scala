...
object ZIOpenTelemetryLayer {
  ...
  /** The OpenTelemetry layer for the ZIOpenTelemetry trait.
    *
    * This is a separate method pulled by the bootstrap layer, as it is used to
    * provide the OpenTelemetry layer to the server options, which are provided
    * by the ZIO application itself. This allows the OpenTelemetry layer to be
    * used
    *
    * @param resourceName
    * @return
    */
  def live(
      withZioMetrics: Boolean
  ): RLayer[OtelProviders & ContextStorage, api.OpenTelemetry] =
    if (withZioMetrics)
