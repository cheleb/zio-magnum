...
object LoggerProvider {
  ...
  /** Provides a logger provider for OpenTelemetry, which logs in OTLP Json
    * format as gRPC if either of the following environment variables is set:
    *   - `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`
    *   - `OTEL_EXPORTER_OTLP_ENDPOINT`
    */
  def grpc(attributes: Attributes): URIO[Scope, Option[SdkLoggerProvider]] =
    OtlpEndpoint("OTEL_EXPORTER_OTLP_LOGS_ENDPOINT") match {
      case None =>
        ZIO.logInfo(
