package com.augustnagro.magnum.ziomagnum.o11y.noop
...
object NoopTracing {
  def apply(instrumentationScopeName: String): Tracing = new Tracing {
    ...
    def addEventWithAttributes(name: String, attributes: Attributes)(implicit
        trace: Trace
    ): UIO[Unit] = ZIO.unit
    ...
    def getCurrentSpanContextUnsafe(implicit trace: Trace): UIO[SpanContext] =
      ZIO.succeed(Span.current().getSpanContext())
    ...
    def scopedEffectFromFuture[A](make: ExecutionContext => Future[A])(implicit
        trace: Trace
    ): Task[A] = ZIO.fromFuture(make)
    ...
    def setAttribute[T](attribute: Attribute[T])(implicit
        trace: Trace
    ): UIO[Unit] = ZIO.unit

    def setAttribute(name: String, value: String)(implicit
        trace: Trace
    ): UIO[Unit] = ZIO.unit

    def setAttribute(name: String, value: Boolean)(implicit
        trace: Trace
    ): UIO[Unit] = ZIO.unit

    def setAttribute(name: String, value: Long)(implicit
        trace: Trace
    ): UIO[Unit] = ZIO.unit

    def setAttribute(name: String, value: Double)(implicit
        trace: Trace
    ): UIO[Unit] = ZIO.unit

    def setAttribute(name: String, values: Seq[String])(implicit
        trace: Trace
    ): UIO[Unit] = ZIO.unit

    def setAttribute[T](key: AttributeKey[T], value: T)(implicit
        trace: Trace
    ): UIO[Unit] = ZIO.unit
