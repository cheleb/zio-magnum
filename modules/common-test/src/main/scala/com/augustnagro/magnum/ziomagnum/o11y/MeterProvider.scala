...
object MeterProvider {
  ...
  private def buildExporter(endpoint: String) = {
    ...
    sys.env
      ...
      .foreach(_ =>
        builder.setAggregationTemporalitySelector(
  ...
  /** Provides a meter provider for OpenTelemetry, which logs in OTLP Json
    ...
    */
  def grpc(attributes: Attributes): URIO[Scope, Option[SdkMeterProvider]] =
    OtlpEndpoint("OTEL_EXPORTER_OTLP_METRICS_ENDPOINT") match {
      case None =>
        ZIO.logInfo(
